...
public static class EfCoreQueryableExtensions
{
    ...
    public static IQueryable<T> ApplySort<T>(
        ...
        IEnumerable<(string sortBy, string sortOrder)>? sorts)
    {
        ...
        foreach (var (sortBy, sortOrder) in sorts)
        {
            ...
            var methodName = (orderedQuery == null ? "OrderBy" : "ThenBy") + (sortOrder.Equals("desc", StringComparison.OrdinalIgnoreCase) ? "Descending" : "");

            var genericMethod = typeof(Queryable).GetMethods().Single(method => method.Name == methodName && method.GetParameters().Length == 2)
                .MakeGenericMethod(typeof(T), propertyInfo.PropertyType);

            orderedQuery = (IOrderedQueryable<T>)genericMethod.Invoke(null, new object[] { orderedQuery ?? source, lambda });
        }
