...
public record CreateLocationStatistic(
    long LocationId,
    string LocationName,
    long CameraId,
    string CameraName,
    ...
    string ColorName,
    string Plate,
    DateTime LprDate) : ITxCreateCommand<CreateLocationStatisticResponse>
{
    public long Id { get; init; } = SnowFlakIdGenerator.NewId();
}

public class CreateLocationStatisticValidator : AbstractValidator<CreateLocationStatistic>
{
    public CreateLocationStatisticValidator()
    {
...
public class CreateLocationStatisticHandler : ICommandHandler<CreateLocationStatistic, CreateLocationStatisticResponse>
{
    private readonly ILogger<CreateLocationStatisticHandler> _logger;
    private readonly IMapper _mapper;
    private readonly IStatisticDbContext _statisticDbContext;

    public CreateLocationStatisticHandler(
        IStatisticDbContext statisticDbContext,
        IMapper mapper,
        ILogger<CreateLocationStatisticHandler> logger)
    {
    ...
    public async Task<CreateLocationStatisticResponse> Handle(
        CreateLocationStatistic command,
        CancellationToken cancellationToken)
    {
        ...
        var plateStatistic = LocationStatistic.Create(
            command.Id,
            locationInformation,
            cameraInformation,
            typeInformation,
        ...
        await _statisticDbContext.LocationStatistics.AddAsync(plateStatistic, cancellationToken: cancellationToken);

        await _statisticDbContext.SaveChangesAsync(cancellationToken);

        var created = await _statisticDbContext.LocationStatistics
        ...
        var locationStatisticDto = _mapper.Map<LocationStatisticDto>(created);

        _logger.LogInformation("Location Statistic a with ID: '{LocationStatisticId} created.'", command.Id);

        return new CreateLocationStatisticResponse(locationStatisticDto);
    }
}
